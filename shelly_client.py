...
class ShellyClient:
    ...
    def _post(self, endpoint: str, params: Dict[str, Any]) -> Optional[Dict]:
        ...
        Inklusive Retry-Logik um 'Flackern' der Verbindung zu verhindern.
        """
                ...
                if response.status_code == 200:
                    try:
                        ...
                        if json_resp.get("isok") == True:
                            ...
                            # Wenn API explizit Fehler meldet (z.B. auth fail), nicht nochmal probieren
                            print(f"Shelly API Error: {json_resp.get('errors')}")
                            return None
                    except ValueError:
                         # Falls Response kein JSON ist, nochmal probieren
                         print(f"Shelly API Fehler: Kein JSON erhalten (Versuch {attempt+1}/{max_retries})")
                else:
                    print(f"Shelly HTTP {response.status_code}: {response.text} (Versuch {attempt+1}/{max_retries})")
                
            except Exception as e:
                print(f"Shelly Connection Error: {e} (Versuch {attempt+1}/{max_retries})")
            
            # Wenn wir hier sind, ist etwas schiefgegangen.
            # Wir warten kurz, bevor wir es nochmal versuchen (Exponential Backoff wäre overkill, einfacher sleep reicht)
            if attempt < max_retries - 1:
                time.sleep(1.0) # 1 Sekunde Pause vor dem nächsten Versuch
                
        # Wenn nach 3 Versuchen immer noch nichts geht:
        return None
